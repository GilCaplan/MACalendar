...
| Board D full scoring | on the deep chain's own objects: count-correct, object P/R/F1 (missing, extra), per-field (title exact/contained, date, start, cadence, reminder, invented a time), harm and destructive errors, restraint (question held back, vague target refused), model-call share with latency, all split by original vs new phrasings (2026-09-25). **Rescored, not re-run**: `python -m assistant.engine.llmjudge.experiments.rescore <checkpoint>…` scores any finished run from its checkpoint in seconds and prints the commit it measured; `--common` restricts several runs to the rows they all share. Checkpoints before 2026-09-25 keep action+title only, so their field and model-call lines read "not recorded". **The default run is the DEEP CHAIN only** (the loop it A/Bs lives there) — its speed lines are not the product's; `--product` runs the front door first, as `Engine.run` does, and records each row's path | LLMJudge |
