...
def test_loop_back_reruns_segment_with_the_mistake(cfg, monkeypatch):
    ...
    It was PINNED TO `old_seg` (2026-09-08), the one segmenter that could
    CHANGE ITS MIND on a re-run; `old_seg` was retired on 2026-09-20. FastSeg
    is deterministic and LLMSeg is off by default, so re-running segmentation
    on unchanged text returns the same items and there is nothing for the
    second pass to fix — the same reason `state.asked_fastrule` exists.
    `assistant/engine/segmentation/ARCHITECTURE.md` records that the mechanism
    is inert while LLMSeg is off. Still xfail(strict), for the reason above.
    """
