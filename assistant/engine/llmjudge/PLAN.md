...
| — | **H1 REFUTED BY FIRE RATE 2026-09-25** (commit `2600059c`, reverted after): built behind `MACALENDAR_RESCUE_SC`, boarded on seeded Board D v2 with the loop on in both arms — fired on **0 of 1,200** TRAIN rows; both arms 1147/1200 (95.6%), fixed 0 / broke 0, p95 6.9 s either way. Segmentation splits a compound before the rescue sees it, so the rescue reads single items and the condition (a STRUCTURE deferral, or a first parse with two or more intents) never holds. | | |
...
| — | **H2 REFUTED BY FIRE RATE 2026-09-22**: 1 row in 1,200 has two rounds, 2 rows differ between arms, all right either way — a selector has nothing to select on this corpus. Stays registered for the v2 set's merged-ask cases only. | | |
...
| — | **H4's CONDITION MEASURED 2026-09-22**: fires on 177 of 1,200 (14.8%), the object right on 92.1% of them; catches 8 of the 40 wrong-operation rows. Ceiling +14 for a perfect model; five of the wrong rows are one deterministic shape (a create frame wrapping a change verb) — cycle 45 on FastRule's board. The model probe runs as the negative control. | | |
...
| — | **H6 MEASURED 2026-09-22**: fires on 10 of 1,200 (0.8%), both arms right on all 10, net 0; costs the p95 gap on looping rows (5.9 → 6.9 s). A latency question; the fair test is the real-usage board. | | |
