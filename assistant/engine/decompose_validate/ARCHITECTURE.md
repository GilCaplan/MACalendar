...
| convention | rule |
|---|---|
| **the date floor** | no day named ⇒ **today** — unless the stated clock has already gone by at the moment of speaking, then **tomorrow** (Gil, 2026-09-22, DEVQA Q42: *"if 5pm passed then we are referring to tomorrow"*; `_rule_passed_clock_means_tomorrow`, and the fast path's SPEC floor applies the same rule so the tracks agree). A NAMED day is never touched by this — that is the past-date bump. The clock is never invented. |
...
| **compact clocks** | "910am", "230PM" are 09:10 and 14:30 anywhere; a bare "830" / "1040" is a clock only after a clock preposition (at, for, from, until, by, around) AND before the end, punctuation or a time/command word — "for 200 people" stays a count. Inside a range ("from 1030 to 1130") the compact form counts like any bare number there. Both tracks carry the same two patterns (`_COMPACT_AP`, `_COMPACT_BARE`; `rule_parser._COMPACT_*_RE`). Real usage, 2026-09-22 (cycle 35): six of the real-usage board's eight remaining generic-title failures had lost a stated time to one of these. Zero fires over the clean corpora (3,000 real minus the sealed 300, FastRule 4,800 train, segmentation 1,592). |
