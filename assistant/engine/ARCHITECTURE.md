...
| ~~**the loop**~~ | **LIVE since 2026-09-10.** `llmjudge/rewrite.py` produces X1' — the failed asks only, reworded — and the orchestrator FREEZES the good objects rather than re-parsing them. It still fails CLOSED: a rewrite whose content words are not all in the transcript is refused, and no rewrite means no loop. |
