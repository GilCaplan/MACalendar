...
<!-- code-stats: {"total_files": 608, "total_lines": 170656} -->
**Generated — do not edit by hand.** The pre-commit hook (`python -m scripts.code_stats --install-hook`) rewrites this whenever the numbers move, so it describes the commit it ships in. By hand: `python -m scripts.code_stats --write`; `--check` says whether it is current, and `tests/unit/test_code_size.py` fails once it is more than 2% out.

**170,656 lines of source across 608 files.** Of that, **94,868 lines are the live product** — the rest is tests, the retired brain, the measurement boards and the dataset tooling.

| category | files | lines |
|---|---:|---:|
| Test code | 185 | 36,811 |
| iOS application | 62 | 23,471 |
| Actions, storage & domain (DB, config, observance, .ics) | 105 | 20,088 |
...
- **Prose** — markdown and the published HTML explainers: 48,587 lines across 117 files.
