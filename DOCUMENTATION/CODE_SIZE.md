...
<!-- code-stats: {"total_files": 610, "total_lines": 170992} -->
**Generated — do not edit by hand.** The pre-commit hook (`python -m scripts.code_stats --install-hook`) rewrites this whenever the numbers move, so it describes the commit it ships in. By hand: `python -m scripts.code_stats --write`; `--check` says whether it is current, and `tests/unit/test_code_size.py` fails once it is more than 2% out.

**170,992 lines of source across 610 files.** Of that, **94,230 lines are the live product** — the rest is tests, the retired brain, the measurement boards and the dataset tooling.

| category | files | lines |
|---|---:|---:|
| Test code | 184 | 36,200 |
| iOS application | 62 | 23,191 |
| Actions, storage & domain (DB, config, observance, .ics) | 105 | 19,885 |
| Engine experiments & boards (not in the answer path) | 62 | 19,722 |
...
- **Prose** — markdown and the published HTML explainers: 48,771 lines across 119 files.
