...
<!-- code-stats: {"total_files": 630, "total_lines": 174423} -->
**Generated — do not edit by hand.** The pre-commit hook (`python -m scripts.code_stats --install-hook`) rewrites this whenever the numbers move, so it describes the commit it ships in. By hand: `python -m scripts.code_stats --write`; `--check` says whether it is current, and `tests/unit/test_code_size.py` fails once it is more than 2% out.

**174,423 lines of source across 630 files.** Of that, **95,930 lines are the live product** — the rest is tests, the retired brain, the measurement boards and the dataset tooling.

| category | files | lines |
|---|---:|---:|
| Test code | 192 | 37,621 |
| iOS application | 62 | 23,507 |
| Actions, storage & domain (DB, config, observance, .ics) | 111 | 20,342 |
| Engine experiments & boards (not in the answer path) | 65 | 20,120 |
| Engine (the brain, shipped path) | 56 | 19,897 |
