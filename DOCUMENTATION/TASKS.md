...
| 100 | **Jude needs `ollama pull nomic-embed-text`** — its retriever hardcodes that model name, so retrieval fails without it even though all five generating roles are pinned to `ollama.model` (llama3.1:8b, verified inside Jude's own resolver). Separately, the local `Torah_RAG/chroma_db` HNSW segment is missing `data_level0.bin` and the index will not load; all 288,737 embeddings are present in `chroma.sqlite3`, so it should be rebuildable rather than re-downloaded. | todo | `assistant/jude/ARCHITECTURE.md` |
